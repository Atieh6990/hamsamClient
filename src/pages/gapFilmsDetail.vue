<template>
  <div class="pageParent" >
    ...
    <div class="detailsParent">
      <img :src="films.PortraitImage" class="poster">
      <div class="Pname">{{ films.Title }}</div>
      <div class="Ename">{{ films.EnglishBody }}</div>
      <div class="catParent">
        <div class="cat">{{ films.Properties[1].Value }}</div>
      </div>
      <div class="duration">{{ films.Properties[0].Value }}</div>
      <div class="description">{{ films.Summary }}</div>
    ...
    <div class="demoParent">
      ...
      <div v-if="showPoster">
        ...
        <img :src="films.LandscapeImage" class="playBox">
...
<!--      </div>-->
    ...
    <div class="line"></div>
        ...
        <div class="Parent-filickty" v-if="recommends[0]">
          <flickity class="flickity onlineFilms" ref="flickity" :options="flickityOptions">

            <div v-for="(item,index) in recommends" class="carousel-cell" :key="index"
            ...
            >
              <img class="img" :src="item.PortraitImage">
              <div class="episode"> {{item.Title}}</div>
            </div>
...
export default {
  ...
  methods: {
    getRecommends() {
      filmsApi.gapFilms(1, 10).then(data => {
    ...
    left() {
      switch (this.yPage) {
        ...
        case 1:
          ...
          if (this.xpos < this.recommends.length - 1) {
            // if (this.xpos < 14) {
            this.xpos++
    ...
    enter() {
      ...
      switch (this.yPage) {
        case 0:
          ...
          if (this.btnIndex == 0) {
            console.log(this.films['WebTvLink'])
            this.openBrowser(this.films['WebTvLink'])
        ...
        case 1:
          ...
          this.resetParams();
          this.films = this.recommends[this.xpos]
